...
def run_cli(args: Optional[List[str]] = None) -> int:
    ...
    parser.add_argument("--vault-add", type=str, metavar="LABEL", help="Save the generated/checked password into the vault under LABEL")
    parser.add_argument("--vault-list", action="store_true", help="List saved vault entries (metadata only)")
    parser.add_argument("--vault-show", type=int, metavar="ID", help="Decrypt and display the vault entry with ID")
    parser.add_argument("--vault-delete", type=int, metavar="ID", help="Delete the vault entry with ID")
    ...
    if parsed.vault_init:
        master = getpass.getpass("Set a new vault master password: ")
    ...
    if parsed.vault_list:
        master = getpass.getpass("Vault master password: ")
        try:
            session = unlock_vault(master)
        ...
        if not entries:
            ...
            for entry in entries:
                tag = f" ({entry.username})" if entry.username else ""
                print(f"[{entry.id}] {entry.label}{tag} - saved {entry.created_at}")
        return 0

    if parsed.vault_show is not None:
        master = getpass.getpass("Vault master password: ")
        try:
            session = unlock_vault(master)
            password = get_entry_password(session, parsed.vault_show)
        except VaultError as e:
            print(f"Error: {e}")
            return 1
        print(password)
    ...
    if parsed.vault_delete is not None:
        master = getpass.getpass("Vault master password: ")
        try:
            session = unlock_vault(master)
            deleted = delete_entry(session, parsed.vault_delete)
        except VaultError as e:
            print(f"Error: {e}")
            return 1
        print("Deleted." if deleted else f"No vault entry with id {parsed.vault_delete}.")
        return 0
    ...
    if parsed.check:
        ...
        if parsed.vault_add:
            ...
            try:
                session = unlock_vault(master)
                entry_id = add_entry(session, parsed.vault_add, parsed.check, username=parsed.username)
            except VaultError as e:
                print(f"Error: {e}")
                return 1
            print(f"Saved to vault as entry [{entry_id}].")
    ...
    if parsed.vault_add and passwords:
        ...
        try:
            session = unlock_vault(master)
            entry_id = add_entry(session, parsed.vault_add, passwords[0], username=parsed.username)
        except VaultError as e:
            print(f"Error: {e}")
            return 1
        print(f"Saved to vault as entry [{entry_id}].")
